...
VulkanTexture::VulkanTexture(VkDevice device, VkPhysicalDevice physicalDevice,
    ...
    : HwTexture(target, levels, samples, w, h, depth, tformat, tusage),
      ...
      mState(handleAllocator->initHandle<VulkanTextureState>(device, allocator, commands, stagePool,
              ...
              getLayerCount(target, depth), any(usage & TextureUsage::PROTECTED))) {
    auto* const state = getSharedState();
    ...
    //Depending on protection level access the right commend buffer
    VulkanCommandBuffer* pCommands = nullptr;
    if (state->mIsProtected) {
        pCommands = &state->mCommands->getProtected();
    } else {
        pCommands = &state->mCommands->get();
    }
    VulkanCommandBuffer& commandsBuf = *pCommands;
